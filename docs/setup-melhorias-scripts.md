...
| **Assinatura/notarização macOS** | Sem certificado Apple Developer (US$ 99/ano); o build macOS sai **sem assinatura** (usuário abre com clique-direito → Abrir) | Ao obter o certificado: criar os secrets `APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_PASSWORD` e `APPLE_TEAM_ID` — o step "Habilitar assinatura macOS" do `release.yml` reativa a assinatura sozinho |
